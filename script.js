...
function performCommand() {
    ...
    if ( preferencesimageCmdPrefvalue.text == "" )
    {
        ...
        if (confirm2("Are you sure you want to kill this process? " + String.fromCharCode(13) + preferencesimageCmdPrefvalue.text + String.fromCharCode(13) +" Do you wish to proceed?"))
        {
             run("taskkill \/IM " + preferencesimageCmdPrefvalue.text + " \/F ");
        }              
    }
...
function backgroundOnMouseDown(Sender,Button,Shift,X,Y)
{
  background.opacity = .001;
  pressed.opacity = 1;
}

function backgroundOnMouseUp(Sender,Button,Shift,X,Y)
{
  background.opacity = 1;
  pressed.opacity = .001;
}
